...
void RenderDevice::setGLRenderStateMatrixMode(nimble::renderdevice::IRenderDevice::eMatrixMode matrixMode){
    if(m_context.m_currentMatrixMode != matrixMode){
        if(matrixMode == RenderDevice::kMatrixModeModel){
            glMatrixMode(GL_MODELVIEW);
        }
        if(matrixMode == RenderDevice::kMatrixModeView){
            glMatrixMode(GL_MODELVIEW);
        }
        if(matrixMode == RenderDevice::kMatrixModeProjection){
...
void RenderDevice::setGLRenderStateTexturing(bool enabled){
	if(enabled){
		//OpenGL ES 1.1 does not support GL_TEXTURE_1D / GL_TEXTURE_3D
